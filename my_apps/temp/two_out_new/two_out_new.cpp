...
class ModuloMessageCondition : public Condition {
 public:
  HOLOSCAN_CONDITION_FORWARD_ARGS(ModuloMessageCondition)
  ModuloMessageCondition() = default;
  ...
  void setup(ComponentSpec& spec) override {
    ...
    spec.param(min_size_,
               "min_size",
		 "Minimum size",
               "Minimum number of messages required to trigger execution",
               static_cast<uint64_t>(1));
    spec.param(period_,
               "period",
               "Period",
               "Execute every N cycles (modulo trigger)",
               1);
  }

  void initialize() override { Condition::initialize(); counter_ = 0; }

  void check(int64_t, SchedulingStatusType* type, int64_t* target_timestamp) const override {
    const bool enough_msgs = check_min_size();
    const bool on_period = (counter_ % period_.get() == 0);
    *type = (enough_msgs && on_period) ? SchedulingStatusType::kReady
                                       : SchedulingStatusType::kWait;
    *target_timestamp = counter_;
  }

  void on_execute(int64_t) override { counter_++; }
...
class TokenApp : public Application {
 public:
  void compose() override {
    ...
    auto op1 = make_operator<Op1>("op1", make_condition<CountCondition>("count_cond", 20));

    // Op2: 每4轮触发一次，必须至少有3条消息
    auto cond1 = make_condition<conditions::ModuloMessageCondition>(
        "cond_in1",
        Arg("receiver", "in1"),
    ...
    // Op3: 每3轮触发一次，至少有2条消息
    auto cond2 = make_condition<conditions::ModuloMessageCondition>(
        "cond_in2",
        Arg("receiver", "in2"),
