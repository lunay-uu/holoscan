...
// 
// op1 produce x tokens/batch every a cycle, op2 consume y token 
//normalization a
//use internal buffer when  x y incompatible
...
class Op2 : public holoscan::Operator {
 ...
 if (buffer.size() < static_cast<size_t>(y_.get()))
    ...
    // 一次 compute 里，可能够多次消费
    while (buffer.size() >= static_cast<size_t>(y_.get())) {
      auto now = std::chrono::system_clock::now();
      auto t = std::chrono::system_clock::to_time_t(now);
      auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
